...
const Home = () => {
    const { fetchWithLogging, loading, error } = useFetch();
    const [orderError, setOrderError] = React.useState(null);

    const handleOrderClick = async () => {
        setOrderError(null);
        try {
            const response = await fetchWithLogging('https://jsonplaceholder.typicode.com/posts', {
                method: 'POST',
    ...
    return (
        <HomeContainer>
            <HomeContent>
                ...
                <Button onClick={handleOrderClick} disabled={loading}>
